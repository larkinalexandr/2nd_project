...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var footerAccordion = document.querySelectorAll('.footer__accordion');
  ...
  if (footerAccordion.length > 0) {
    for (var i = 0; i < footerAccordion.length; i++) {
      ...
      footerAccordion[i].classList.add('is-closed');
      footerAccordion[i].addEventListener('click', function (evt) {
        if (evt.target.tagName === 'H3') {
          openAccordion(evt);
        }
      });
      footerAccordion[i].addEventListener('keydown', function (evt) {
        if (evt.key === 'Enter') {
          openAccordion(evt);
        }
      });
  ...
  var aboutTextWrapper = document.querySelector('.about__text-wrapper');
  var aboutParagraph = aboutTextWrapper.querySelectorAll('p');
  ...
  [].forEach.call(document.querySelectorAll('.tel'), function (input) {
    var MAX_NUMBER_LENGTH = 11;

    var prefixNumber = function prefixNumber() {
      return '+7 (';
    };
    ...
    input.addEventListener('input', function () {
      ...
      for (var _i = 0; _i < value.length && _i < MAX_NUMBER_LENGTH; _i++) {
        switch (_i) {
          case 0:
            number += prefixNumber();
            continue;
